...
const tabs = [
    {
        id: 'content',
        title: 'Brand',
        hint: 'Name, logo and contact details',
        sections: [
    ...
    },
    {
        id: 'settings',
        title: 'Social',
        hint: 'Footer and sharing profiles',
        sections: [
    ...
    },
    {
        id: 'seo',
        title: 'SEO',
        hint: 'Default meta tags for the site',
        sections: [
...
export default function SiteSettings({ site, contact, social, seo }) {
    ...
    function renderSection(section) {
        ...
        if (section.id === 'seo') {
            return <SeoField value={form.data.seo} onChange={(next) => form.setData('seo', next)} previewUrl="https://arsoftbd.com" />;
        }
    ...
    return (
        <AdminLayout title="Company Information" subtitle="Manage brand identity, contact details, social links and SEO defaults.">
            <ResourceEditorShell
                title="Site Settings"
                subtitle={form.data.site.name || 'Global website settings'}
                tabs={tabs}
                onSubmit={submit}
